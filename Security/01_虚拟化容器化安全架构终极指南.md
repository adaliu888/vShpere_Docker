...
| 领域 | 证据目录建议 | 关键文件 | 说明 |
|---|---|---|---|
| vSphere/审计 | `artifacts/YYYY-MM-DD/` | `vc_events.json`、`tasks.csv`、`manifest.json` | 事件/任务与哈希校验 |
| NSX/策略 | `artifacts/YYYY-MM-DD/` | `nsx_policies.json`、命中Top-N报表 | 发布前后快照与对比 |
| K8s/策略 | `artifacts/YYYY-MM-DD/runtime/` | `policies/*.yaml`、`audit/*.json`、`events/*.json` | NetworkPolicy/PSA/审计 |
| 镜像/供应链 | `artifacts/YYYY-MM-DD/images/` | `sbom/*.json`、`signatures/*.sig`、`attestations/*.jsonl` | SBOM/签名/声明 |
| 监控/日志/追踪 | `artifacts/YYYY-MM-DD/observability/` | `metrics/*.csv`、`logs/*.json`、`traces/*.json` | 观测性证据留存 |
| 性能/基准 | `artifacts/YYYY-MM-DD/perf/` | 原始数据、图表、`manifest.json` | 与性能指南一致 |

> 统一锚点：版本与标准全部参考《2025年技术标准最终对齐报告.md》；结构示例参见 `vShpere_VMware/09_安全与合规管理/Artifacts_Index.md`。
