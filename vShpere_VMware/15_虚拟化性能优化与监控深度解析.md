...
> 版本锚点与性能证据（新增）：本篇涉及 vSphere/ESXi/vCenter/NSX 等版本统一参考《2025年技术标准最终对齐报告.md》。性能监控与调优产物（原始数据、报表、图表）请归档到 `artifacts/YYYY-MM-DD/perf/`，包含 `manifest.json` 与 `*.sha256`，与 `Analysis/04_性能分析与优化综合指南.md` 的约定保持一致。
