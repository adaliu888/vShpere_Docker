...
> 版本锚点与性能证据（新增）：本文涉及 vSphere/vSAN/存储阵列等版本统一参考《2025年技术标准最终对齐报告.md》。存储性能测试与优化产物请归档至 `artifacts/YYYY-MM-DD/perf/`（原始 IOPS/吞吐/延迟数据、图表、`manifest.json` 与 `*.sha256`），与 `Analysis/04_性能分析与优化综合指南.md` 一致。
