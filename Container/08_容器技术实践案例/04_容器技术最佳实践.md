...
- 目标：将镜像、策略与运行时证据标准化落盘，支撑合规审计与追溯。
- 目录对齐：参见 `vShpere_VMware/09_安全与合规管理/Artifacts_Index.md` 中“Kubernetes 合规证据模板”。
- 最小落地清单：
  - 镜像供应链：SBOM（Syft/CycloneDX）、签名（Cosign）、attestations（SLSA）
  - 策略与基线：NetworkPolicy/PSA/RBAC/Quota 等 YAML 与变更记录
  - 审计与事件：kube-apiserver 审计、集群事件快照
  - CIS 报告：`cis-benchmark-report.json` 记录分数与整改项
- 标准锚点：统一引用《2025年技术标准最终对齐报告.md》；需要更新时仅改锚点文档。
